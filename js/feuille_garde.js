/* ============================================================
   SYSTEL POMPIERS - MODULE FEUILLE DE GARDE (v18)
   Créable uniquement par SOG/CDG et ADMIN
   ============================================================ */
...
function renderFeuilleGarde() {
  ...
  container.innerHTML = `
    <div class="fg-toolbar">
      <div class="fg-select-group"><label>Engin :</label>
        <select id="fg-engin-select" onchange="chargerFeuilleGarde()">${enginsOptions}</select>
      </div>
      <div class="fg-select-group"><label>Date :</label>
...
function chargerFeuilleGarde() {
  ...
  if (!feuille) {
    container.innerHTML = `<div class="fg-empty">
      ...
      <div style="font-weight:700;color:var(--text-muted);">Aucune feuille de garde pour ${engin?.nom} le ${date}</div>
      ${canEdit ? `<button class="btn btn-primary" style="margin-top:15px;" onclick="creerFeuilleGarde()">+ Créer la feuille de garde</button>` : '<p style="color:#a0aec0;margin-top:10px;font-size:13px;">Contactez un SOG/CDG ou un administrateur.</p>'}
    </div>`;
    return;
  }
          ...
          <tbody>
            ${(feuille.equipe||[]).map((eq,idx) => `
              <tr>
                ...
                <td>${eq.grade}</td>
                <td>${canEdit?`<input type="text" value="${eq.fonction||''}" onchange="majFonctionEquipier('${enginId}','${date}',${idx},this.value)" style="width:150px;padding:4px 8px;">`:(eq.fonction||'--')}</td>
                ${canEdit?`<td><button class="btn btn-danger btn-sm" onclick="supprimerEquipier('${enginId}','${date}',${idx})">✕</button></td>`:''}
              </tr>`).join('')}
          </tbody>
        </table>
        ${canEdit ? `
          <div class="fg-add-row" style="margin-top:15px;">
            <select id="fg-new-equipier">${personnelsOptions}</select>
            <input type="text" id="fg-new-fonction" placeholder="Fonction (ex: Chef d'agrès, Pilote...)" style="flex:1;padding:8px 12px;border:1px solid var(--border-color);border-radius:6px;">
...
function ajouterEquipier(enginId, date) {
  ...
  sauvegarderDonnees(); chargerFeuilleGarde();
}
...
function validerFeuille(enginId, date) {
  ...
  sauvegarderDonnees(); chargerFeuilleGarde(); showToast("Feuille validée !");
}